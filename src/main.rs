use srvcs_disjoint::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-disjoint listening");
    let deps = Deps {
        intersection_url: cfg.intersection_url.clone(),
    };
